...
export function featureRecall (
  ...
  stimulusTypeShownFirst: StimulusKind,                                          // only used in split trials
    ...
    // 1. Get only the displayStimuli rows that match this logical trial
    ...
    // 2. Extract the stimuliData column and flatten it, also add the information about whether it was shown first (left) or second (right)
    const fullStimulusList = rows.values().flatMap(row => row.stimuliData);
